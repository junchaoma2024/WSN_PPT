...
值得注意的是，不确定性同样存在于层次任务网络规划中。华中科技大学王红卫、祁超团队将不确定HTN规划中的不确定性归纳为三类：\textbf{状态不确定}、\textbf{动作效果不确定}和\textbf{任务分解不确定}。这一分类框架将第3章的HTN方法与本章的概率规划方法有机关联——HTN的层次分解结构可与概率推理相结合，为应急响应等不确定性环境下的复杂任务提供更具表达力的规划模型。
