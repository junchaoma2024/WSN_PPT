...
\begin{remark}
国内大语言模型近年来发展迅速，百度``文心一言''（ERNIE Bot）、阿里``通义千问''（Qwen）、智谱``ChatGLM''、深度求索``DeepSeek''等在中文理解和推理能力上持续提升。秦龙等在《自动化学报》上提出了``基于大语言模型的复杂任务自主规划处理框架''，探索了利用国产大模型进行任务规划的技术路径。赵鑫等编著的《大语言模型》一书为中文读者提供了系统的LLM技术参考。
\end{remark}
...
国内电商物流的智能仓储规模世界领先。京东``地狼''仓储机器人系统、菜鸟``无人仓''、极智嘉（Geek+）等已在大规模仓储中实现了多机器人协同拣选，日处理订单可达百万级。这些系统的任务规划核心正在从传统的规则引擎向AI驱动的方向演进。
\end{appbox}
