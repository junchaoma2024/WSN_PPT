...
华中科技大学祁超教授团队在HTN规划方面开展了深入研究，提出了面向复杂管理任务的智能规划方法。
...
\subsubsection{祁超团队的研究贡献}

根据祁超教授团队的研究，HTN规划在以下方面得到扩展：

\begin{enumerate}[leftmargin=2em]
    \item \textbf{不完全信息处理}：在信息缺失情况下进行任务规划
    \item \textbf{并发任务支持}：处理可并行执行的子任务
    \item \textbf{不确定持续时间}：考虑任务执行时间的不确定性
    \item \textbf{资源约束集成}：将资源限制纳入规划过程
\end{enumerate}

代表性成果包括：
\begin{itemize}[leftmargin=2em]
    \item 《基于层次任务网络规划的应急响应决策理论与方法》（科学出版社，2015）
    \item "Hierarchical Task Network-based Emergency Task Planning with Incomplete Information, Concurrency and Uncertain Duration"（Knowledge-Based Systems, 2016）
\end{itemize}
...
\begin{enumerate}[leftmargin=2em, label={[\arabic*]}]
    ...
    % HTN规划
    \item 王红卫, 祁超. 基于层次任务网络规划的应急响应决策理论与方法[M]. 北京: 科学出版社, 2015.
    \item Qi C, Wang H, et al. Hierarchical Task Network-based Emergency Task Planning with Incomplete Information, Concurrency and Uncertain Duration[J]. Knowledge-Based Systems, 2016, 112:69-79.
